...
void DSelector_thrown::Init(TTree *locTree)
{
	...
	dOutputFileName = "gen.root"; //"" for none
	...
	dPreviousRunNumber = 0;
        ...
        dHist_massX = new TH1F("massX","Mass;M(pi0 eta);Events per 0.01 GeV",70,0.5,1.2);
	dHistThrownTopologies = new TH1F("hThrownTopologies","hThrownTopologies", 20, -0.5, 19.5);
...
Bool_t DSelector_thrown::Process(Long64_t locEntry)
{
	...
	//Loop over throwns
        vector<TLorentzVector> secondaryPhotons;
	for(UInt_t loc_i = 0; loc_i < Get_NumThrown(); ++loc_i)
	{
		...
		//cout << "Thrown " << loc_i << "(Parent Idx " << dThrownWrapper->Get_ParentIndex() << "): " << locPID << ", " << locThrownP4.Px() << ", " << locThrownP4.Py() << ", " << locThrownP4.Pz() << ", " << locThrownP4.E() << endl;
                if ( locPID == 1 && dThrownWrapper->Get_ParentIndex() != -1 ){
                    secondaryPhotons.push_back(locThrownP4);
                }
	}

        TLorentzVector resonance1;
        TLorentzVector resonance2;
        if (secondaryPhotons.size()==4){
            resonance1 = secondaryPhotons[0]+secondaryPhotons[1];
            resonance2 = secondaryPhotons[2]+secondaryPhotons[3];
            dHist_massX->Fill((resonance1+resonance2).M());
        }
